...
### Next session — Day 21 priorities

- [ ] **WhatsApp DJ: pass neuro profile instead of recovery buckets.** The conversational DJ currently passes raw recovery score to the LLM with a crude 4-bucket interpretation (<40% "rough day", 40-60% midrange, 60-80% "decent", 80%+ "great day, fully charged"). This defeats the entire purpose of the 12-signal neuro profile. On April 7 (81% recovery, bad sleep), the DJ would tell the LLM "Great day — fully charged. Full energy is fair game" — the exact wrong calibration. Fix: compute the continuous profile in `whatsapp/handler.py` via `compute_continuous_profile()`, translate the profile + key z-scores into plain-English body context for the LLM prompt (e.g., "body needs grounding and calming today — REM was low, sleep was fragmented, despite high recovery"). User intent still drives selection; body context calibrates it. Files: `whatsapp/handler.py` (`_get_whoop_context`), `intelligence/nl_song_selector.py` (`_build_whoop_context`, `_WHOOP_CONTEXT_TEMPLATE`).

- [ ] **Product question: should playlists match how you feel, or what your body needs?** April 8: user feels happy and energetic (great REM), but deep sleep was low (z=-1.114). The profile produces a nearly-balanced warm playlist, not an energy playlist. Is that right? Two interpretations: (1) the playlist should match subjective feeling — if you feel great, give you energy. (2) the playlist should account for what the body still needs — deep sleep was bad, so don't push full energy even if mood is high. This is a product design question, not a calibration question. It determines whether the system optimizes for "how do I feel right now?" or "what does my body need right now?" — and those diverge on days like April 8. Needs a deliberate decision with implications for how we weight deep sleep vs REM going forward.

- [ ] **Song rotation is broken — gap formula too lenient.** The gap formula `log2(library_size / 150)` gives only 3 days for a 963-song library. Result: songs appear every 3-5 days. Saathiyaa and Tum Ho have appeared 10 times each across 65 playlists. Today's playlist has 7 songs that appeared just 2 days ago (Apr 5). The hard cap blocked 16 songs but the remaining pool is still the same ~60 top-scoring songs cycling endlessly. Two issues: (1) the formula divides by 150, shrinking the gap — with 963 songs, `log2(963)` = 9.9 but `log2(963/150)` = 2.7 → rounds to 3. The `/150` is too aggressive. (2) "current banger" discount reduces the gap by 1 more day (to 2), meaning high-engagement songs can appear every other playlist. Need to investigate: is the divisor wrong? Should it be `log2(library_size)` directly? What gap feels right for a ~1000-song library? Files: `matching/query_engine.py` (`compute_min_repeat_gap`, `get_days_since_last_appearance`), `db/queries.py` (`get_days_since_last_appearance`).

- [ ] **WhatsApp DJ: song finder mode — "what's that song?"** The DJ already knows the user's full library (1,188 songs with artist, album, genre, mood, era). New use case: user has a vague memory of a song — "there's a party song from Meghna Mishra" or "that slow Arijit song from a 2016 movie" — and the DJ finds it. The LLM already sees the full library; it just needs to be prompted to handle identification queries, not only playlist requests. Should ask clarifying questions if multiple matches ("Did you mean X or Y?"), surface 2-3 candidates with enough context (artist, album, mood) for the user to pick. This is a natural extension of the conversational DJ — same architecture, new intent type. The insight: people often know what they want to hear but can't recall the name. An LLM with their full library can close that gap through conversation.
