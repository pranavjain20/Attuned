...
## WHOOP Insights — Recovery vs How You Actually Feel

### The observation (Day 9, Mar 25)

Today: Recovery 83% (HRV 55ms), but sleep was mediocre — 6.9h total, 2.8h D+R, 88% efficiency. Two days ago had nearly identical sleep (6.2h, 2.9h D+R) but 38% recovery. The 83% feels like it oversells how rested the user actually is. Yesterday had 58% recovery but 8.1h with 4.1h D+R and felt fresher.

Pattern: WHOOP's recovery is HRV-dominant. Sleep architecture doesn't proportionally influence the score. When HRV and sleep quality diverge, the recovery score follows HRV, but the user's subjective experience follows sleep.

### Research findings

**HRV is a biomarker, not a cause of how you feel.**
- Laborde, Mosley & Thayer (2017): HRV is an *index* of self-regulatory capacity, not a direct cause of subjective states. The neurovisceral integration model frames it as a readout of prefrontal cortex regulation.
- Thayer et al. (2012): HRV is a peripheral marker of central autonomic network function. It reflects modulation, it doesn't produce the experience.
- The causal chain runs: good sleep → healthy autonomic balance → high HRV. Not: high HRV → feel good.

**HRV and sleep architecture recover through independent mechanisms.**
- Grimaldi et al. (2019, PMC6369727): After sleep restriction, HRV rebounded during recovery sleep but slow-wave sleep did not differ from baseline. Vagal rebound happened within the first hour via parasympathetic activation, independent of sleep stage restoration.
- HRV can rebound from stress resolution (vagal surge), hydration restoration, or baroreflex compensation — none of which require good sleep architecture.

**What actually determines how you feel when you wake up (hierarchy):**
1. Sleep architecture (deep + REM) — physical restoration, adenosine clearance, glymphatic drainage, emotional processing
2. Sleep continuity — fragmented sleep is worse than shorter consolidated sleep (Finan et al. 2015)
3. Circadian alignment — waking during deep sleep at wrong phase = severe grogginess (Wertz et al. 2006)
4. Cortisol awakening response — morning alertness signal, HPA axis, NOT measured by WHOOP
5. HRV / autonomic balance — facilitates recovery but doesn't drive the subjective feeling

**WHOOP's recovery algorithm is ~85% HRV.**
- Independent reverse-engineering (Marco Altini / HRV4Training, Quantified Scientist): HRV alone predicts recovery score with r = 0.85-0.93. Sleep performance (duration vs need) is a modifier, not a primary driver. Sleep stage composition (deep/REM ratios) does NOT directly feed into the score.
- A simple logistic regression on HRV percentile + RHR percentile + sleep performance % reproduces WHOOP recovery within ±5 points for ~80% of days.

**The dissociation is common and well-documented:**
- Athletes show green recovery but feel terrible (parasympathetic overshoot from overtraining — Plews et al. 2013)
- Best workouts on red days (positive sympathetic activation reads as "bad")
- Alcohol gives green recovery (GABAergic parasympathetic shift) while person feels awful
- Illness onset not caught until 12-24 hours after symptoms start
- Late meals tank recovery via elevated RHR despite person feeling fine
- HRV-subjective wellness correlation in athletes is "moderate at best" (Bellenger et al. 2016)

**Sleep architecture predicts subjective state better than HRV:**
- Vitale et al. (2015): Sleep architecture metrics (deep sleep %) predicted next-morning wellness scores more strongly than nocturnal HRV.
- Hynynen et al. (2011): Correlation between morning HRV and subjective recovery was weak (r ≈ 0.2-0.3). Sleep quality self-reports correlated more strongly.
- Dijk (2009): SWS quantity directly determines subjective physical recovery. SWS deprivation produces fatigue even when total sleep time is adequate.
- Wassing et al. (2020): Selective REM suppression increases next-day negative affect by ~60% amygdala hyperactivity.

### What this means for Attuned

1. **The state classifier's priority ordering is correct** — sleep deficits (P2-P4) fire before recovery-based states (P5) regardless of recovery score. This aligns with the research.
2. **The continuous baseline scaling needs a sleep quality input** — currently uses recovery delta only, which is HRV-driven. Today's case: +25pp delta → energy lean, but mediocre sleep means the user doesn't feel that energetic.
3. **WHOOP's blind spot is Attuned's opportunity** — WHOOP compresses HRV, RHR, and sleep into one number that's 85% HRV. Attuned decomposes them and gives sleep architecture independent weight. This is the core value proposition beyond what WHOOP alone provides.
4. **Recovery score may be largely redundant** — since HRV explains 85% of it, and we already use HRV directly in baselines and trends, the recovery score adds marginal value. The real signal is in sleep architecture + HRV independently.

### Next step: Sleep quality dampener for baseline

When recovery is high but sleep quality is poor (today's case), the baseline profile should be dampened — less energy lean, more balanced. The recovery delta says "energy up" but sleep says "not really." The system should respect both signals.

Design pending — same pattern as the restorative sleep gate but in reverse direction.
