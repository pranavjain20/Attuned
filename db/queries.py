...
def update_song_metadata_batch(
    conn: sqlite3.Connection,
    metadata: list[dict],
) -> None:
    """Batch update duration_ms and release_year for multiple songs."""
    conn.executemany(
        "UPDATE songs SET duration_ms = COALESCE(?, duration_ms), release_year = COALESCE(?, release_year) WHERE spotify_uri = ?",
        [(m.get("duration_ms"), m.get("release_year"), m["uri"]) for m in metadata],
    )
    conn.commit()
...
def get_recent_playlist_track_uris(
    ...
    for days_ago in range(days, 0, -1):  # Process oldest first so newest overwrites
        ...
        if row and row["track_uris"]:
            for uri in json.loads(row["track_uris"]):
                result[uri] = days_ago

    return result


def get_days_since_last_appearance(
    conn: sqlite3.Connection,
    before_date: str,
    max_lookback: int = 7,
) -> dict[str, int]:
    ...
    Returns dict of {uri: days_since_last}. Tracks not in any recent playlist
    are not included.
    """
    from datetime import datetime, timedelta
